...
b<-ggplot(dataset4)+
  geom_line(data=dataset4 %>% filter(cause_type=="C"),aes(x=hora,y=n),size=1.6,color="#e7593e")+
  geom_line(data=dataset4 %>% filter(cause_type=="S"),aes(x=hora,y=n),size=1.6,color="#59bee4")+
  geom_line(data=dataset4 %>% group_by(hora) %>% summarise(n=sum(n)) %>% ungroup(),aes(x=hora,y=n),size=2,color="#6b9565")+
